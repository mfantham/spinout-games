export function canMove(dials: boolean[], n: number): boolean {
...
function legalMoves(dials: boolean[]): number[] {
  const moves: number[] = [];
...
function toggleDial(dials: boolean[], n: number): boolean[] {
  return dials.map((dial, index) => (index === n - 1 ? !dial : dial));
}

function serialize(dials: boolean[]): string {
  return dials.map((dial) => (dial ? "1" : "0")).join("");
}

export function isSolved(dials: boolean[]): boolean {
  return dials.every(Boolean);
}

export function solutionLength(n: number): number {
  return Math.floor(2 ** (n + 1) / 3);
}

export function nextHintMove(dials: boolean[]): number | null {
  if (isSolved(dials)) {
    return null;
  }
  ...
  const queue: { state: boolean[]; firstMove: number | null }[] = [
    { state: dials, firstMove: null },
  ];
